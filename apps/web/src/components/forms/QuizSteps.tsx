...
export function QuizSteps({ active }: { active: 'details' | 'questions' }) {
  const steps = [
    ...
    <div className="mb-6 flex flex-wrap items-center gap-2 rounded-2xl border border-[#e9ece9] bg-[#f8faf9] p-2">
      {steps.map((step) => {
        ...
        return (
          <div
            key={step.key}
            className={`flex items-center gap-2 rounded-xl border px-3 py-2 text-xs font-semibold shadow-sm sm:text-sm ${isActive ? 'border-[#d69a5b] bg-[#f5ebdd] text-[#5a321f] shadow-[0_2px_8px_rgb(90_50_31_/_0.12)]' : 'border-transparent text-neutral-500'}`}
          >
