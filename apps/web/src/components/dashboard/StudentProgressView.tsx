    ...
    <div className="mx-auto max-w-6xl space-y-5">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => <button key={item.enrollmentId} type="button" onClick={() => { void openDetails(item); }} className="rounded-2xl border border-[#e3c9ae] bg-[#fff8ef] p-5 text-left shadow-sm transition-colors hover:bg-[#fdf0e1] hover:shadow-md"><div className="truncate text-sm font-medium text-[#5a321f]" title={item.studentName || 'Unnamed student'}>{item.studentName || 'Unnamed student'}</div><div className="mt-1 truncate text-sm font-normal text-neutral-600" title={item.courseName}>{item.courseName}</div><div className="mt-4"><ProgressCell item={item} /></div><div className="mt-4 text-xs text-neutral-500">Enrolled {new Date(item.enrollmentDate).toLocaleDateString()}</div></button>)}
        </div>
      )}
