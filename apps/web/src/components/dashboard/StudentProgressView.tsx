...
export function StudentProgressView({ apiPath }: Props) {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-[#fffdf9] px-6 py-6 text-neutral-900">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <input
          ...
          className="min-w-0 flex-1 rounded-xl border border-[#ead8c6] bg-white px-4 py-3 text-sm text-neutral-900 outline-none placeholder:text-neutral-400 focus:border-[#7a4a2e] focus:ring-2 focus:ring-[#a8784f]/20 sm:max-w-2xl"
        />
        <Select
          ...
          className="sm:w-44"
          buttonClassName="rounded-xl border-[#ead8c6] bg-white px-4 py-3 text-base text-neutral-900 shadow-sm focus:border-[#7a4a2e] focus:ring-2 focus:ring-[#a8784f]/20"
        />
      ...
      {loading ? (
        ...
        <div className="grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => <button key={item.enrollmentId} type="button" onClick={() => { void openDetails(item); }} className="min-w-0 w-full rounded-2xl border border-[#d9b894] bg-[#fff3e0] p-5 text-left shadow-md transition-colors hover:bg-[#ffead0] hover:shadow-lg"><div className="truncate text-sm font-medium text-[#5a321f]" title={item.studentName || 'Unnamed student'}>{item.studentName || 'Unnamed student'}</div><div className="mt-1 break-words text-sm font-normal text-neutral-600" title={item.courseName}>{item.courseName}</div><div className="mt-4 min-w-0"><ProgressCell item={item} /></div><div className="mt-4 text-xs text-neutral-500">Enrolled {new Date(item.enrollmentDate).toLocaleDateString()}</div></button>)}
        </div>
      )}
      ...
      {selected && <div className="fixed inset-0 z-50 flex justify-end bg-black/50" onClick={() => setSelected(null)}><aside className="h-full w-full max-w-xl overflow-y-auto bg-white p-6 text-neutral-900 shadow-xl" onClick={(event) => event.stopPropagation()}><button type="button" onClick={() => setSelected(null)} className="float-right text-2xl text-neutral-500" aria-label="Close">x</button><h2 className="text-xl font-semibold">{selected.studentName || 'Unnamed student'}</h2><p className="mt-1 text-sm text-neutral-500">{selected.studentEmail}</p><h3 className="mt-6 font-semibold">{selected.courseName}</h3><ProgressCell item={selected} />{detailLoading ? <div className="mt-6"><Spinner size="md" label="Loading progress details..." /></div> : <><div className="mt-6 space-y-3"><h3 className="font-semibold">Module progress</h3>{selected.modules.map((module) => <div key={module.id} className="flex items-center justify-between rounded-lg bg-[#fffdf9] px-3 py-2 text-sm"><span>{module.title}</span><span className="font-medium">{module.percentage}% {module.complete ? '✓' : ''}</span></div>)}</div><div className="mt-6 space-y-2"><h3 className="font-semibold">Quiz status</h3>{selected.quizzes.map((quiz) => <div key={quiz.quizId} className="flex justify-between text-sm"><span>{quiz.attempted ? (quiz.passed ? 'Passed' : 'Failed') : 'Not attempted'}</span><span>{quiz.attemptsUsed} used, {quiz.attemptsRemaining ?? 'unlimited'} remaining</span></div>)}</div></>}</aside></div>}
      </div>
    </div>
  );
}
