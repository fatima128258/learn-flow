        ...
        {loading ? <div className="space-y-4"><Skeleton variant="text" height={28} /><Skeleton variant="text" height={28} /><Skeleton variant="text" height={28} /></div>
          ...
          : <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-neutral-200">
                ...
                <tbody className="divide-y divide-neutral-100">{categories.map((category) => <tr key={category.id} className="cursor-pointer text-sm hover:bg-neutral-50" onClick={() => setSelectedCategory(category)}><td className="px-4 py-4 font-medium text-neutral-900">{category.name}</td><td className="max-w-xs px-4 py-4 text-neutral-600"><span className="block max-w-xs truncate">{category.description || '—'}</span></td><td className="px-4 py-4 text-neutral-600">{category.courseCount}</td><td className="max-w-xs px-4 py-4 text-neutral-600"><span className="line-clamp-2">{category.instructors?.length ? category.instructors.map((instructor) => instructor.name).join(', ') : '—'}</span></td><td className="px-4 py-4"><Badge variant={category.status === 'ACTIVE' ? 'success' : 'default'} size="sm">{category.status}</Badge></td><td className="px-4 py-4" onClick={(event) => event.stopPropagation()}><CategoryActionsMenu onView={() => setSelectedCategory(category)} onEdit={() => setEditing(category)} onDelete={() => setDeleting(category)} /></td></tr>)}</tbody>
              </table>
            </div>}
