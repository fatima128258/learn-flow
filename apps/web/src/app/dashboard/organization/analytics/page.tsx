...
import { EmptyState, EmptyStateIcons, ErrorState, Select, Spinner } from '@/components/ui';
        ...
        <>
          <section aria-labelledby="enrollment-analytics-title" className="mb-8">
            ...
            <ChartCard
              ...
              action={<Select aria-label="Enrollment date range" value={String(range)} onChange={(value) => setRange(Number(value) as 7 | 30 | 90 | 365)} options={enrollmentRanges.map((item) => ({ value: String(item.value), label: item.label }))} />}
            >
                ...
                {Object.entries(enrollmentsByMonth).map(([month, monthlyEnrollments]) => (
                  <section key={month}>
                    ...
                    <div className="space-y-3 sm:hidden">
                      {monthlyEnrollments.map((enrollment) => (
                        <article key={enrollment.id} className="rounded-xl border border-neutral-200 bg-white p-4 shadow-sm">
                          <div className="flex items-start justify-between gap-4">
                            <div className="min-w-0">
                              <p className="text-xs font-medium uppercase tracking-wide text-neutral-500">Student</p>
                              <p className="mt-1 break-words font-medium text-neutral-900">
                                {enrollment.student.name?.trim() || enrollment.student.email}
                              </p>
                            </div>
                            <div className="shrink-0 text-right">
                              <p className="text-xs font-medium uppercase tracking-wide text-neutral-500">Purchase date</p>
                          ...
                          <div className="mt-4 border-t border-neutral-100 pt-3">
                            <p className="text-xs font-medium uppercase tracking-wide text-neutral-500">Course purchased</p>
                            <p className="mt-1 break-words text-sm text-neutral-700">{enrollment.course.name}</p>
                          </div>
                        </article>
                      ))}
                    </div>
