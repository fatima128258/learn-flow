...
async function deleteLearnFlowAnchor() {
  try {
    console.log('🔍 Finding all "LearnFlow Anchor" organizations...\n');
    
    // Find all organizations with this name
    const organizations = await prisma.organization.findMany({
      where: {
        name: 'LearnFlow Anchor'
      },
      include: {
        users: {
          include: {
    ...
    if (organizations.length === 0) {
      console.log('❌ No organizations found with name "LearnFlow Anchor"');
      return;
    }
    
    console.log(`✅ Found ${organizations.length} organization(s) with name "LearnFlow Anchor"\n`);
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`⚠️  WARNING: This will delete ALL ${organizations.length} "LearnFlow Anchor" organization(s) and their related data!`);
    ...
    console.log(`\n✅ All ${organizations.length} "LearnFlow Anchor" organization(s) deleted successfully!`);
