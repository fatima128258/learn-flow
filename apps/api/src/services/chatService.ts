...
export async function open(orgId: string, courseId: string, userId: string, role?: string, requestedStudentId?: string) {
  ...
  return repo.findForCourse(courseId, studentId, instructorId).then((existing) => existing || repo.createConversation(courseId, studentId!, instructorId));
}

export async function list(userId: string) { return repo.listConversations(userId); }

async function participant(id: string, userId: string) {
  const c = await repo.findConversation(id);
  if (!c) throw new Error('CONVERSATION_NOT_FOUND');
  if (c.studentId !== userId && c.instructorId !== userId) throw new Error('FORBIDDEN');
  return c;
}
export async function messages(id: string, userId: string, limit = 50, cursor?: string) {
  await participant(id, userId);
...
export async function send(id: string, userId: string, content: string) {
  const c = await participant(id, userId);
...
export async function read(id: string, userId: string) { await participant(id, userId); return repo.markRead(id, userId); }
export async function block(id: string, userId: string) { await participant(id, userId); return repo.blockConversation(id, userId); }
export async function removeMessage(id: string, userId: string) { return repo.deleteMessage(id, userId); }
