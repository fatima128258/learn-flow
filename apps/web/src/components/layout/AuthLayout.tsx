...
export const AuthLayout: React.FC<AuthLayoutProps> = ({ children, hideChrome = false, noScroll = false }) => {
  return (
    <div className={`flex min-h-screen flex-col overflow-x-hidden bg-[#fff9f0] ${noScroll ? 'lg:h-screen' : ''}`}>
      ...
      <main className={`relative flex min-h-0 min-w-0 flex-1 justify-center overflow-x-hidden px-4 py-8 sm:px-6 lg:px-8 ${noScroll ? 'items-center overflow-y-auto lg:py-4' : 'items-start overflow-y-auto'}`}>
        ...
        <div className="relative mx-auto w-full min-w-0 max-w-6xl animate-fade-in">
          <div className="mb-8 flex justify-center lg:hidden">
            <LearnFlowLogo href="/" tone="dark" size={38} />
          </div>
          <div className="grid min-w-0 overflow-hidden rounded-[28px] border border-[#ead8c6] bg-[#fffdf9] shadow-[0_24px_70px_rgba(90,50,31,0.12)] lg:grid-cols-2">
            <AuthVisual />
            <div className="min-w-0 px-6 py-8 sm:px-12 sm:py-10 lg:px-16">{children}</div>
          </div>
        </div>
      </main>
